...
class PatternDetector:
        ...
        if (isinstance(self.config, list)):
            ...
            for index, config_pattern in enumerate(self.config):
                ...
                if ('name' not in config_pattern):
                    print(
                        "ERROR - REQUIRED NAME NOT SET FOR THE PATTERN NUMBER " + str(index + 1))
                    exit()
                if ('sounds' not in config_pattern):
                    print("ERROR - REQUIRED SOUNDS NOT FILLED IN FOR THE PATTERN `" +
                          config_pattern['name'] + "`")
                    exit()
                ...
                def throttle_detect(
                    self, pattern_name=pattern_name): return self.detect_throttle(pattern_name)

                def detect(self): return False

                def throttle_activate(
                    self, pattern_name=pattern_name): return self.throttle(pattern_name)
                ...
                if ('throttle' in config_pattern):
                    ...
                    throttle_activate = lambda self, throttles=copy(
                        config_pattern['throttle']): self.activate_throttle(throttles)

                detection_calls = []
                if ('threshold' in config_pattern):
                    thresholds = copy(config_pattern['threshold'])
                    detection_calls = self.generate_detection_functions(
                        thresholds, sounds)
                ...
                if ('continual_threshold' in config_pattern):
                    ...
                    continual_detection_calls = self.generate_detection_functions(
                        thresholds, sounds)

                    recovery_threshold = RECORD_SECONDS * 6

                    def cdc_lambda(self, detection_calls=copy(
                        continual_detection_calls)): return self.detect_all(detection_calls)

                    def dc_lambda(self, detection_calls=copy(
                        detection_calls)): return self.detect_all(detection_calls)

                    def detect_lambda(self, pattern_name=pattern_name,
                                      rt=recovery_threshold): return self.detect_throttle(pattern_name, rt)
                    detection_calls = [lambda self, cdc=copy(cdc_lambda), dc=copy(
                        dc_lambda), dt=copy(detect_lambda): cdc(self) if dt(self) else dc(self)]

                def detect(self, detection_calls=copy(detection_calls)
                           ): return self.detect_all(detection_calls)
    ...
    def generate_detection_functions(self, thresholds, sounds):
        ...
        for i in range(times):
            ...
            if ('percentage' in thresholds):
                detection_calls.append(lambda self, dictIndex=index, threshold=thresholds['percentage'], sounds=sounds: sum(
                    self.predictionDicts[dictIndex][sound]['percent'] for sound in sounds) >= threshold)
            if ('probability' in thresholds):
                detection_calls.append(lambda self, dictIndex=index, threshold=thresholds['probability'], sounds=sounds: sum(
                    self.predictionDicts[dictIndex][sound]['probability'] for sound in sounds) >= threshold)
            if ('power' in thresholds):
                detection_calls.append(
                    lambda self, dictIndex=index, threshold=thresholds['power']: self.predictionDicts[dictIndex]['silence']['power'] >= threshold)
            if ('ratio' in thresholds and len(sounds) > 1):
                detection_calls.append(lambda self, dictIndex=index, threshold=thresholds['ratio'], sounds=sounds: (
                    self.predictionDicts[dictIndex][sounds[0]]['percent'] / self.predictionDicts[dictIndex][sounds[1]]['percent'] >= threshold))
            if ('intensity' in thresholds):
                detection_calls.append(
                    lambda self, dictIndex=index, threshold=thresholds['intensity']: self.predictionDicts[dictIndex]['silence']['intensity'] >= threshold)
            if ('frequency' in thresholds):
                detection_calls.append(
                    lambda self, dictIndex=index, threshold=thresholds['frequency']: self.predictionDicts[dictIndex]['silence']['frequency'] >= threshold)

        # These will check whether a threshold is below the given value
        if ('below_percentage' in thresholds):
            detection_calls.append(lambda self, threshold=thresholds['below_percentage'], sounds=sounds: sum(
                self.lastDict[sound]['percent'] for sound in sounds) < threshold)
        if ('below_power' in thresholds):
            detection_calls.append(
                lambda self, threshold=thresholds['below_power']: self.lastDict['silence']['power'] < threshold)
        if ('below_ratio' in thresholds and len(sounds) > 1):
            detection_calls.append(lambda self, threshold=thresholds['below_ratio'], sounds=sounds: (
                self.lastDict[sounds[0]]['percent'] / self.lastDict[sounds[1]]['percent'] < threshold))
        if ('below_intensity' in thresholds):
            detection_calls.append(
                lambda self, threshold=thresholds['below_intensity']: self.lastDict['silence']['intensity'] < threshold)
        if ('below_frequency' in thresholds):
            detection_calls.append(
                lambda self, threshold=thresholds['below_frequency']: self.lastDict['silence']['frequency'] < threshold)
    ...
    def detect_strategy(self, action, config):
        ...
        if (strategy == 'single_tap'):
            ...
            if (detected and 'above_frequency' in config):
                detected = self.above_frequency(
                    lastDict, config['above_frequency'])

            if (detected and 'below_frequency' in config):
                detected = self.below_frequency(
                    lastDict, config['below_frequency'])
    ...
    def winner_over(self, probabilityData, labelA, labelB):
        return probabilityData[labelA]['percent'] > probabilityData[labelB]['percent']
    ...
    def combined_percentage(self, probabilityData, labels, percentage):
        ...
        return combinedPercent >= percentage
