        ...
        if (nextMode not in self.__modes):
            ...
            # Use data/code for new users
                ...
                print("Could not find " + nextMode + ", does the " +
                      nextMode + ".py file exist in the data/code folder?")
                print("------------------------------")
                exit()
            else:
            ...
            for classname, actualclass in clsmembers:
                if (actualclass.__module__ == full_module_name):
                    module_found = True
                    self.__modes[nextMode] = actualclass(
                        ModeSwitcher.__instance, self.__is_testing)
            ...
            if (module_found):
                ...
                print("")
                print("---- MODE NOT FOUND ERROR ----")
                print("The file " + nextMode +
                      ".py does not contain a valid class.")
                print(
                    "Make sure it looks like one of the examples in the docs/examples folder.")
                print(
                    "For more information on how modes work, look in the docs/TUTORIAL.md file.")
                print("------------------------------")
                exit()
                return False
        else:
