...
def persist_srt_file(srt_filename: str, events: List[DetectionEvent]):
    ...
    with open(srt_filename, 'w') as srt_file:
        for index, event in enumerate(events):
            ...
            srt_file.write(ms_to_srt_timestring(event.start_ms) +
                           " --> " + ms_to_srt_timestring(event.end_ms) + '\n')
...
def parse_srt_file(srt_filename: str, rounding_ms: int, show_errors: bool = True) -> List[TransitionEvent]:
    ...
    with open(srt_filename, "r") as srt:
        ...
        for line_index, line in enumerate(srt):
            if not line.strip():
                ...
                time_pair = [timestring.strip()
                             for timestring in line.split("-->")]
                time_start = math.ceil(srt_timestring_to_ms(
                    time_pair[0]) / rounding_ms) * rounding_ms

                time_end = math.ceil(srt_timestring_to_ms(
                    time_pair[1]) / rounding_ms) * rounding_ms
            elif not line.strip().isnumeric():
                if type_sound == "":
                    type_sound = line.strip()
                    if time_start < time_end:
                        positive_event_list.append(
                            str(time_start) + "---" + type_sound + "---start")
                        positive_event_list.append(
                            str(time_end) + "---" + type_sound + "---end")
                    elif show_errors:
                        print(".SRT error at line " + str(line_index) +
                              " - Start time not before end time! Not adding this event - Numbers won't be valid!")
    ...
    for time_index, time_event in enumerate(positive_event_list):
        ...
        if time_index != 0 and len(transition_events) > 0 and transition_events[-1].start_index == math.floor(int(time_event.split("---")[0]) / rounding_ms):
            if show_errors:
                print("Found duplicate entry at second " + str(math.floor(int(
                    time_event.split("---")[0]) / rounding_ms) / 1000) + " - Not adding duplicate")
            continue

        if time_event.endswith("---start"):
            if time_index == 0 and int(time_event.split("---")[0]) > 0:
                transition_events.append(
                    TransitionEvent(BACKGROUND_LABEL, 0, 0))
            ...
            transition_events.append(TransitionEvent(time_event.split(
                "---")[1], math.floor(ms_start / rounding_ms), ms_start))
        elif time_event.endswith("---end"):
            ms_start = math.floor(int(time_event.split("---")[0]))
            transition_events.append(TransitionEvent(
                BACKGROUND_LABEL, math.floor(ms_start / rounding_ms), ms_start))
...
def count_total_label_ms(label: str, base_folder: str, rounding_ms: int) -> int:
    ...
    if os.path.isdir(segments_dir):
        srt_files = [x for x in os.listdir(segments_dir) if os.path.isfile(os.path.join(
            segments_dir, x)) and x.endswith(".v" + str(CURRENT_VERSION) + ".srt")]
        for srt_file in srt_files:
            total_ms += count_label_ms_in_srt(
                label, os.path.join(segments_dir, srt_file), rounding_ms)
    return total_ms
...
def print_detection_performance_compared_to_srt(actual_frames: List[DetectionFrame], frames_to_read: int, srt_file_location: str, output_wave_file=None):
    ...
    for frame in actual_frames:
        ...
        if output_wave_file is not None:
            ...
            detection_signal = np.full(
                int(frames_to_read / 4), int(signal_strength))
        ...
        if expected == actual:
            ...
            if false_detections > 0:
                ...
                for ei in range(first_index - 1, current_event_index):
                    ...
                    event_end = transition_events[event_index + 1].start_index if event_index + 1 < len(
                        transition_events) else len(actual_frames) - 1
    ...
    for false_type in false_types:
        ...
        false_types[false_type]["avg"] = round(
            np.mean(false_types[false_type]["data"])) if amount > 0 else 0
        false_types[false_type]["std"] = round(
            np.std(false_types[false_type]["data"])) if amount > 0 else 0
    ...
    export_row.append("0%" if total_occurrences == 0 else str(
        round(positive_recognitions / total_occurrences * 100)) + "%")
    print("Found:                       " + str(positive_recognitions) + " (" +
          ("0%" if total_occurrences == 0 else str(round(positive_recognitions / total_occurrences * 100)) + "%)"))
    print("Missed:                      " + str(false_recognitions) + " (" +
          ("0%" if total_occurrences == 0 else str(round(false_recognitions / total_occurrences * 100)) + "%)"))
    print("------------- Frame data -------------")
    print("Total frames:               " + str(len(actual_frames)))
    export_row.append(
        str(round((ms_true_positive + ms_true_negative) / total_ms * 1000) / 10) + "%")
    print("Accuracy:                   " + export_row[-1])
    print("-------- Positive / negative --------")
    export_row.append(
        str(round(ms_true_positive / total_ms * 1000) / 10) + "%")
    print("True positive:              " + export_row[-1])
    export_row.append(
        str(round(ms_true_negative / total_ms * 1000) / 10) + "%")
    print("True negative:              " + export_row[-1])
    export_row.append(
        str(round(ms_false_positive / total_ms * 1000) / 10) + "%")
    print("False positive:             " + export_row[-1])
    export_row.append(
        str(round(ms_false_negative / total_ms * 1000) / 10) + "%")
    ...
    if ms_false_positive > 0:
        for fp_type in [{"key": "false_start", "name": "Early start"}, {"key": "missed_dip", "name": "Missed dip"}, {"key": "late_stop", "name": "Late stop"}, {"key": "full_false_positive", "name": "Full FP"},]:
            ...
            print((fp_type["name"] + " (% of FP):").ljust(key_length, " ") + ("0%" if ms_false_positive == 0 else str(
                round(ms_total / ms_false_positive * 100)) + "%") + " (" + str(false_types[fp_type["key"]]["times"]) + "x)")
            print("  [ Average " + str(false_types[fp_type["key"]]["avg"]) +
                  "ms (σ " + str(false_types[fp_type["key"]]["std"]) + "ms) ]")
            export_row.append(str(false_types[fp_type["key"]]["times"]))
            export_row.append(str(false_types[fp_type["key"]]["avg"]) + " σ " + str(
                false_types[fp_type["key"]]["std"]) if false_types[fp_type["key"]]["times"] > 0 else "0")
    else:
    ...
    if ms_false_negative > 0:
        ...
        for fn_type in [{"key": "lag", "name": "Lagged start"}, {"key": "stutter", "name": "Stutter"}, {"key": "cutoff", "name": "Early cut-off"}, {"key": "full_miss", "name": "Full miss"},]:
            ...
            print((fn_type["name"] + " (% of FN):").ljust(key_length, " ") + ("0%" if ms_false_negative == 0 else str(
                round(ms_total / ms_false_negative * 100)) + "%") + " (" + str(false_types[fn_type["key"]]["times"]) + "x)")
            print("  [ Average " + str(false_types[fn_type["key"]]["avg"]) +
                  "ms (σ " + str(false_types[fn_type["key"]]["std"]) + "ms) ]")
            export_row.append(str(false_types[fn_type["key"]]["times"]))
            export_row.append(str(false_types[fn_type["key"]]["avg"]) + " σ " + str(
                false_types[fn_type["key"]]["std"]) if false_types[fn_type["key"]]["times"] > 0 else "0")
    else:
        export_row.extend(["0", "0", "0", "0", "0", "0", "0", "0"])
    print("--------------------------------------")
