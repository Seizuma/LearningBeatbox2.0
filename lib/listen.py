        ...
        if (len(audio_frames) >= 2):
            ...
            if (high_speed == True and highestintensity < SILENCE_INTENSITY_THRESHOLD):
                ...
                power = get_recording_power(
                    fftData, classifier.get_setting('RECORD_SECONDS', RECORD_SECONDS))
            ...
            with open(replay_file, 'a', newline='') as csvfile:
                ...
                while (listening_state['currently_recording'] == True):
                    if (not listening_state['classifierQueue'].empty()):
                        ...
                        replay_row = {'time': int(seconds_playing * 1000) / 1000, 'actions': ':'.join(
                            actions), 'buffer': listening_state['classifierQueue'].qsize()}
                        for label, labelDict in probabilityDict.items():
                            replay_row[label] = labelDict['percent']
                            if (labelDict['winner']):
                                replay_row['winner'] = label
                            replay_row['intensity'] = int(
                                labelDict['intensity'])
...
def start_nonblocking_listen_loop(classifier, mode_switcher=False, persist_replay=False, persist_files=False, amount_of_seconds=-1, high_speed=False):
    ...
    actionConsumer = threading.Thread(name='action_consumer', target=action_consumer, args=(
        listening_state['stream'], classifier, dataDicts, persist_replay, replay_file, mode_switcher))
...
def predict_wav_files(classifier, wav_files):
    ...
    for index, wav_file in enumerate(wav_files):
        highestintensity = get_highest_intensity_of_wav_file(
            wav_file, classifier.get_setting('RECORD_SECONDS', RECORD_SECONDS))
        probabilityDict, predicted, frequency = predict_wav_file(
            wav_file, classifier, highestintensity)
...
def predict_wav_file(wav_file, classifier, intensity):
    # FEATURE ENGINEERING
    data_row, frequency = feature_engineering(wav_file, classifier.get_setting(
        'RECORD_SECONDS', RECORD_SECONDS), classifier.get_setting('FEATURE_ENGINEERING_TYPE', FEATURE_ENGINEERING_TYPE))
    data = [data_row]
...
def create_empty_probability_dict(classifier, data, frequency, intensity, power):
    ...
    for label in classifier.classes_:
        ...
        probabilityDict[label] = {'probability': probability,  'percent': percent, 'intensity': int(
            intensity), 'winner': winner, 'frequency': frequency, 'power': power}
        index += 1

    if ('silence' not in classifier.classes_):
        probabilityDict['silence'] = {'probability': 1.0, 'percent': 100, 'intensity': int(
            intensity), 'winner': True, 'frequency': frequency, 'power': power}
...
def create_probability_dict(classifier, data, frequency, intensity, power):
    ...
    for index, percent in enumerate(percentages[0]):
        ...
        probabilityDict[label] = {'probability': probabilities[0][index], 'percent': percent, 'intensity': int(
            intensity), 'winner': index == predicted, 'frequency': frequency, 'power': power}

    if ('silence' not in classifier.classes_):
        probabilityDict['silence'] = {'probability': 1.0, 'percent': 100, 'intensity': int(
            intensity), 'winner': False, 'frequency': frequency, 'power': power}
...
def load_running_classifier(classifier_name):
    if (classifier_name != "dummy"):
        print("Loading classifier " + CLASSIFIER_FOLDER +
              "/" + classifier_name + ".pkl")
        classifier = joblib.load(
            CLASSIFIER_FOLDER + "/" + classifier_name + ".pkl")
