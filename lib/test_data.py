...
def test_data(with_intro):
    ...
    if (len(available_models) == 0):
        ...
        print(
            "Please train an algorithm first using the [L] option in the main menu")
        return
    elif (with_intro):
        print("-------------------------")
    ...
    analyze_replay_or_audio(
        available_models, available_replays, available_sounds)
...
def replay(available_replays):
    if (len(available_replays) == 1):
        ...
        for index, replay in enumerate(available_replays):
            ...
            print("- [" + str(index + 1) + "] - " + replay +
                  " (" + str(filesize / 1000) + "kb)")
            replay_prompt = (index + 1) % 10 == 0 or index + \
                1 == len(available_replays)
            if (replay_prompt):
                replay_index = input("")
                if (replay_index == ""):
                    continue
    ...
    plot_replay(pd.read_csv(REPLAYS_FOLDER + "/" +
                replay_file, skiprows=0, header=0))
...
def plot_audio(folder):
    ...
    for index, wav_file in enumerate(full_wav_files):
        features, frequency = feature_engineering(
            wav_file, RECORD_SECONDS, FEATURE_ENGINEERING_TYPE)
        ...
        print("Analyzing file " + str(index + 1) +
              " - Intensity %0d - Frequency: %0d           " % (intensity, frequency), end="\r")
...
def audio_analysis(available_models):
    ...
    if (new_or_existing.lower() == "n"):
        ...
        print("-------------------------")
        seconds = input(
            "How many seconds of audio should we record? ( default is 15 )")
        if (seconds == ""):
            seconds = 15
        else:
            seconds = int(seconds)
        ...
        replay_file = start_nonblocking_listen_loop(
            classifier, False, True, True, seconds, True)
        classifier = None
        print("-------------------------")
        ...
        with open(REPLAYS_FOLDER + '/analysis-replay-' + str(time.time()) + '.csv', 'a', newline='') as csvfile:
            headers = ['time', 'winner', 'intensity',
                       'frequency', 'power', 'actions', 'buffer']
            ...
            for index, prediction in enumerate(predictions):
                timeString = full_wav_files[index].replace(
                    REPLAYS_AUDIO_FOLDER + os.sep, "").replace(".wav", "")
                dataRow = {'time': int(float(timeString) * 1000) / 1000,
                           'intensity': 0, 'power': 0, 'actions': [], 'buffer': 0}
...
def choose_classifier(available_models):
    if (len(available_models) == 1):
        ...
        classifier_file_index = input(
            "Type the number of the model that you want to test: ")
        if (classifier_file_index == ""):
            classifier_file_index = 0
        elif (int(classifier_file_index) > 0):
            classifier_file_index = int(classifier_file_index) - 1

    classifier_file = CLASSIFIER_FOLDER + "/" + \
        available_models[classifier_file_index]
    ...
    print("This model can detect the following classes: " +
          ", ".join(classifier.classes_))
...
def test_accuracy(available_models, available_sounds):
    ...
    threshold = input(
        "At what percentage should we consider a sound accurately detected? ")
    if (threshold == ""):
        threshold = 0
    else:
    ...
    for index, sound in enumerate(available_sounds):
        if (sound in classifier.classes_ or (MICROPHONE_SEPARATOR and sound.split(MICROPHONE_SEPARATOR)[0] in classifier.classes_)):
            ...
            recordings_dir = os.path.join(
                RECORDINGS_FOLDER + "/", sound + "/source")
            ...
            for prediction in predictions:
                ...
                if (sound in prediction and prediction[sound]['winner'] and prediction[sound]['percent'] >= threshold):
                    i_correct += 1
            print("Accuracy above threshold %0d - %0.1f " %
                  (threshold, round((i_correct / i) * 100.0)), end="\n")

    total_metrics = precision_recall_fscore_support(
        true_wav_file_labels, predicted_wav_file_labels, average=None, labels=classifier.classes_, beta=0.5)
    print("Output label".ljust(30) + " P       R       F0.5    Samples")
    print("----------------------------------------------------------------")
    for index, sound in enumerate(classifier.classes_):
        print(sound.ljust(30) + " %0.2f    %0.2f    %0.2f    %0d" %
              (total_metrics[0][index], total_metrics[1][index], total_metrics[2][index], total_metrics[3][index]))
    print("----------------------------------------------------------------")
    print("Total percentiles")
    print("50th".ljust(30) + " %0.2f    %0.2f    %0.2f" %
          (np.percentile(total_metrics[0], 50), np.percentile(total_metrics[1], 50), np.percentile(total_metrics[2], 50)))
    print("75th".ljust(30) + " %0.2f    %0.2f    %0.2f" %
          (np.percentile(total_metrics[0], 75), np.percentile(total_metrics[1], 75), np.percentile(total_metrics[2], 75)))
    print("95th".ljust(30) + " %0.2f    %0.2f    %0.2f" %
          (np.percentile(total_metrics[0], 95), np.percentile(total_metrics[1], 95), np.percentile(total_metrics[2], 95)))
    test_data(True)
...
def analyze_replay_or_audio(available_models, available_replays, available_sounds):
    ...
    if (replay_or_audio.lower() == "r"):
        if (len(available_replays) == 0):
            print(
                "No replays to be analyzed yet - Make sure to do a practice run using the play mode first")
            analyze_replay_or_audio(
                available_models, available_replays, available_sounds)
        else:
...
def plot_replay(replay_data):
    ...
    if ('expected' in replay_data.drop([], axis=1)):
        plt.subplot(2, 1, 1)
        plt.title("Percentage correct vs incorrect", loc='left',
        ...
        expected_actual_frame = pd.DataFrame(
            {'correct': correct_frames, 'wrong': wrong_frames, 'silence': silence_frames})
        ...
        for column in expected_actual_frame:
            if (column != "silence"):
                if (column == "wrong"):
                    ...
                    label = 'Correct'
                num += 1
                plt.bar(np.arange(replay_data['time'].size), expected_actual_frame[column],
                        color=color, linewidth=1, alpha=0.9, label=label, bottom=bottom)
        ...
        # Add percentage plot
        plt.subplot(2, 1, 1)
        plt.title("Percentage distribution of predicted sounds",
        ...
        for column in replay_data.drop(['winner', 'intensity', 'time', 'frequency', 'actions', 'buffer', 'power'], axis=1):
            if (column != "silence"):
                color = colors[num]
                num += 1
                plt.bar(np.arange(replay_data['time'].size), replay_data[column],
                        color=color, linewidth=1, alpha=0.9, label=column, bottom=bottom)
    ...
    ax1.bar(np.arange(replay_data['time'].size), np.array(
        replay_data['intensity']), color='green', linewidth=1)

    frequencyAxis = ax1.twinx()
    frequencyAxis.plot(np.arange(
        replay_data['time'].size), replay_data['frequency'], '-', color='red')
...
def plot_bars(learning_data, index):
    ...
    validation_accuracy = str(
        int(learning_row['validation_accuracy'] * 1000) / 10)

    # Add percentage plot
    string_epoch = str(int(learning_row['epoch'] + 1))
    plt.title("Epoch " + string_epoch + " - Validation accuracy " + validation_accuracy +
              " - Percentage distribution of sounds", loc='left', fontsize=12, fontweight=0, color='black')
    plt.ylabel("Percentage")
    for column in learning_data.drop(['epoch', 'validation_accuracy', 'loss'], axis=1):
        color = colors[num]
        num += 1
        plt.bar(num, learning_row[column] * 100,
                color=color, linewidth=1, alpha=0.9, label=column)
