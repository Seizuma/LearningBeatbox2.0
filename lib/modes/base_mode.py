...
class BaseMode:
    ...
    def __init__(self, modeSwitcher, is_testing=False, repeat_delay=REPEAT_DELAY, repeat_rate=REPEAT_RATE):
        self.inputManager = InputManager(
            is_testing=is_testing, use_direct_keys=self.use_direct_keys, input_release_lag_ms=self.input_release_lag * 1000)
        self.mode = "regular"
        ...
        if (SPEECHREC_ENABLED == True and IS_WINDOWS == True):
            ...
            self.simpleCommandRule = SimpleSpeechCommand(
                self.speech_commands, callback=self.toggle_speech)
    ...
    def handle_input(self, dataDicts):
        ...
        if (self.mode == "speech"):
            if (IS_WINDOWS == True):
                python_compiler.PumpWaitingMessages()
    ...
    def detect(self, key):
        # print(f"Detect called with key: {key}")
        # print(f"Current toggles: {self.toggles}")
